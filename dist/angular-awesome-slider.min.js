...
* License: MIT 
**/
!function(a){"use strict";a.module("angularAwesomeSlider",[]).directive("slider",["$compile","$templateCache","$timeout","$window","slider",function(b,c,d,e,f){return{restrict:"AE",require:"?ngModel",scope:{options:"=",ngDisabled:"="},priority:1,link:function(g,h,i,j){function k(){var b=a.element(e),c=function(a){g.slider.onresize()};b.bind("resize",c),g.$on("$destroy",function(){b.unbind("resize",c)})}if(j){if(!g.options)throw new Error('You must provide a value for "options" attribute.');a.injector();
...
// disabling
var o=function(a){g.disabled=a,g.slider&&(g.tmplElt.toggleClass("disabled"),g.slider.disable(a))};g.$watch("ngDisabled",function(a){o(a)}),g.limitValue=function(b){return g.options.modelLabels?a.isFunction(g.options.modelLabels)?g.options.modelLabels(b):void 0!==g.options.modelLabels[b]?g.options.modelLabels[b]:b:b},
// Turn off all events in associated with the namespace
g.$on("$destroy",function(){g.slider&&g.slider.domNode&&(g.slider.domNode.find("*").off(),g.slider.domNode.off(),a.element(document).off("click.angular-awesome-slider").off("mousedown.angular-awesome-slider").off("mousemove.angular-awesome-slider").off("mouseup.angular-awesome-slider").off("mousedown.angular-awesome-slider"))});var p=function(a,b,c){return new f(a,b,c)}}}}}]).config(function(){}).run(function(){})}(angular),function(a){"use strict";a.module("angularAwesomeSlider").constant("sliderConstants",{SLIDER:{settings:{from:1,to:40,step:1,smooth:!0,limits:!1,round:!1,value:"3",dimension:"",vertical:!1,calculate:!1,onstatechange:!1,callback:!1,realtime:!1},className:"jslider",selector:".jslider-",css:{visible:{visibility:"visible"},hidden:{visibility:"hidden"}}},EVENTS:{}})}(angular),function(a){"use strict";a.module("angularAwesomeSlider").factory("sliderUtils",["$window",function(a){return{offset:function(a){
