...
var extractors = {
    ...
    symlink: function (file, destination, zip) {
    ...
    copy: function (file, destination, zip) {
        var type;
        ...
        .then(function (buffer) {
            var linkTo = buffer.toString();
            return stat(path.resolve(file.parent, linkTo))
            .then(function (stats) {
                if (stats.isFile()) {
                    type = 'File';
                    var input = new stream.Readable();
                    input.wrap(fs.createReadStream(path.resolve(file.parent, linkTo)));
